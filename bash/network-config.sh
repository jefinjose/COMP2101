...
lan_address=$(ip a s $address | awk '/inet /{gsub(/\/.*/,"");print $2}')
lan_hname=$(getent hosts $lan_address | awk '{print $2}')
external_ip=$(curl -s icanhazip.com)
external_name=$(getent hosts $external_ip | awk '{print $2}')
router_address=$(ip r s default | awk '{print $3}')
router_hname=$(route | grep "default" | awk '{print $2}')
...
LAN Hostname    : $lan_hname
External IP     : $external_ip
External Name   : $external_name
Router Address  : $router_address
Router Hostname : $router_hname
EOF
